      ...
      <main className="main">
        ...
        <section className="heroCards">
          ...
          <Metric
            ...
            value={weak[0]?.name.split(' ').slice(0, 3).join(' ')}
      ...
      <div className="focusGrid">
        {weak.map((w) => (
          <div className="focus" key={w.id} style={{ borderColor: w.color }}>
            <b>{w.name}</b>
            <span>{w.rate}% score • {w.done}/{w.total} practiced</span>
          </div>
        ))}
      </div>
...
function StudyGuide() {
  const [open, setOpen] = useState(data.studyGuide.domains[0].id);
  const dom = data.studyGuide.domains.find((d) => d.id === open);

  return (
    <section className="guideLayout">
      <div className="guideMenu">
        {data.studyGuide.domains.map((d) => (
          <button
            key={d.id}
      ...
      {reveal && (
        <div className="explain">
          ...
          {q.options.map((opt) => (
            ...
            >
              <b>{opt.key}</b>
              <span>{q.explanations[opt.key]}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {examStarted && (
        <div>
          <div className="examProgress">
            <span>{answered}/{list.length} answered</span>
            <span>
              {examSubmitted
...
function Flashcards() {
  ...
  const dom = data.domains.find((d) => d.id === card.domain);
  ...
  return (
    <section className="panel center">
      ...
      <p className="lead">
        Card {i + 1} of {data.flashcards.length} • {dom.name}
      </p>
      ...
      <div className="pager centerPager">
        ...
        <button
          onClick={() => {
            setI((i + 1) % data.flashcards.length);
            setFlip(false);
          }}
        >
          Next
    ...
    <section className="panel">
      ...
      {stats.map((s) => (
        <div className="stat" key={s.id}>
          <div>
            <b>{s.name}</b>
            <span>{s.done}/{s.total} answered • {s.correct} correct • {s.missed} missed</span>
          </div>

          <strong>{s.rate}%</strong>

          <div className="bar">
            <i style={{ width: `${Math.max(3, s.rate)}%`, background: s.color }} />
          </div>
        </div>
      ))}
    </section>
